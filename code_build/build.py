...
class TsdatPipelineBuild():
    # I think this build runs from the $CODEBUILD_SRC_DIR folder because that is the location 
    ...
    def find_changed_tsdat_pipelines(self) -> List[str]:
        """----------------------------------------------------------------------------------------
        Use git diff to find out which pipelines have had modifications from the latest commit.
        
        Raises:
            Exception: If the git diff command fails.

        Returns:
            List[str]: List of pipeline names who have had code changes in the latest commit.
        ----------------------------------------------------------------------------------------"""
        changed_pipelines = []
        command = f"cd {Env.PIPELINES_REPO_PATH} && {Env.AWS_REPO_PATH}/cdk/build/find_modified_pipelines.sh"
        ...
        if completed_process.returncode == 0:
            ...
            changed_pipelines = output.strip().split()  # Parse the newline separated text into a list
    ...
    def build_base_image(self):
        """----------------------------------------------------------------------------------------
        build the base Docker image that is shared by all pipelines.  This build runs first.
        ----------------------------------------------------------------------------------------"""
